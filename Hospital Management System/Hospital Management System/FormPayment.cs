...
namespace Hospital_Management_System
{
    public partial class FormPayment : Form
    {
        ...
        private void loadDgv()
        {
            ...
            {
                dgvPayment.Rows.Add(item.item, "Rp. " + item.nominal, item.notes, btnDelete.Text = "Delete" ,item.payment.card_holder_name, item.payment.primary_account_number, item.payment.expiration_date, item.payment.service_code, item.payment.total_payment);
            }
        }
        ...
        private void dgvPayment_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 & e.RowIndex < dgvPayment.Rows.Count)
            {
                ...
                tbTotalPayment.Text = "Rp." + dgvPayment.Rows[e.RowIndex].Cells[8].Value?.ToString();
            }
        }
